...
class NetworkTopologyBuilder:
    ...
    def build_random_mesh(sim, num_nodes : int, extra_edges: int):
        ...
        Erdos-Renyi: each edge has a fixed probability of being present or absent, independently of the other edges
        """
        ...
        # Add extra connectivity without self loops
        ...
        for i in range(num_nodes):
            if i == 0 :
                ...
                sim.origin_server = i
            elif i < num_nodes // 10: # Top 10% become edge caches
                sim.node_metadata[i] = {"type": "edge_cache"}
            else:
                sim.node_metadata[i] = {"type": "client"}
    ...
    def build_regional_clusters(sim, regions: list[str], nodes_per_region: int):
        ...
        # Connect regions together via "backbone" routers (Inter-region)
        ...
        # Select one node as origin node
    ...
    def build_scale_free_network(sim, num_nodes, m=2):
        ...
        core_edge = m * (m - 1)
        new_edge = 2 * m * (num_nodes - m)
        total_slots = core_edge + new_edge
        ...
        # Start with a small fully connected core of 'm' nodes (Called hubs)
        for i in range(m):
        ...
        # For each newly added node select m previous node without replacement (no repeat) based on degree of the node
        # so we directly sample it in our repeated_node array for O(1) sampling
        ...
        for idx, (node_id, degree) in enumerate(sorted_hubs):
            if idx == 0:
                ...
                sim.origin_server = i
            elif idx < num_nodes // 10: # Top 10% become edge caches
